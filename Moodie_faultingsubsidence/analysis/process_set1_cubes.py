...
if __name__ == '__main__':
    ...
    MODEL_SET = os.path.join(os.path.expandvars('$SCRATCH'), 'faults_set1')
    ...
    if len(spinups_fldrs) == 1:
        spinups_fldrs = spinups_fldrs * 7
    ...
    if _input_flag == 'debug':
        ...
        _ntsspinups = SPINUPS_SAMPLE.shape[0] // 5
    ...
    matrix_time_idx = utilities.define_time_idx_set(
        CUBES_SAMPLE, start=0, num=_ntsmatrix, irregular=False)
    ...
    def process_spinup_job(_path, i, time_idx, pool):
        ...
        # compute masks, used in analysis below
    ...
    def process_matrix_job(_path, i, time_idx, pool):
        ...
        # cat everything into a table
        results = pd.concat(
            [scalar_data, delta_mask_data, subsd_mask_data, spacetime_data,
             topset_data, flux_data],
            axis=1)

        return results

    # some hacky junk
    #   need a consistent area for subsidence, set it up here
    CUBES = []
    for i, matrix_fldr in enumerate(matrix_fldrs):
        _path = os.path.join(matrix_fldr, 'pyDeltaRCM_output.nc')
        CUBES.append(dm.cube.DataCube(_path))
    utilities._hacky_make_masks(CUBES)
    CUBES = None
    ...
    with Pool(n_workers) as pool:
        ...
        spinups_long.to_csv(
            os.path.join('output_tables', 'set1_spinups_table.csv'),
            index=True)
        ...
        matrix.to_csv(
            os.path.join('output_tables', 'set1_matrix_table.csv'),
            index=True)
    ...
    summ_tbl = pd.DataFrame(columns=['sigma', 'Qw', 'Qs', 'Hbf', 'B', 'qw'])
    for i, matrix_fldr in enumerate(matrix_fldrs):
        _path = os.path.join(matrix_fldr, 'pyDeltaRCM_output.nc')
    ...
    summ_tbl.to_csv(
        os.path.join('output_tables', 'set1_metasummary_table.csv'),
        index=True)
